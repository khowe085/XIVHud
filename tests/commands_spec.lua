...
describe("commands", function()
  ...
  describe("copy", function()
    it("carries the character name through with its original case", function()
      local action = parse("copy", "Azureblood", "confirm")
      assert.are.same({ action = "copy", character = "Azureblood", confirmed = true }, action)
    end)

    it("reports a missing confirmation rather than acting", function()
      assert.are.same({ action = "copy", character = "Azureblood", confirmed = false }, parse("copy", "Azureblood"))
    end)

    it("requires a character", function()
      assert_error(parse("copy"), "character")
    end)

    it("rejects a trailing word that is not confirm", function()
      assert_error(parse("copy", "Azureblood", "yes"), "confirm")
    end)
  end)
