...
describe("core", function()
  ...
  describe("copying another character's configuration", function()
    local widget

    before_each(function()
      env.fs.put("data/Alpha/bar.lua", "return { slots = { default = { pos = { x = 40, y = 50 } } } }")
      env.fs.put("data/Alpha/core.lua", 'return { snap = 25, slot = "default" }')
      env.fs.put("data/Alpha/bar/extra.lua", "return { deep = true }")
      widget = core.register(bar("bar", 100, 200))
      login("Azureblood")
    end)

    it("asks for confirmation before overwriting anything", function()
      core.on_command({ "copy", "Alpha" })
      assert.is_not_nil(env.said():lower():find("confirm"))
      assert.is_nil(env.fs.files["data/Azureblood/bar.lua"], "nothing may be written yet")
      assert.are.same({ 100, 200 }, widget.pos)
    end)

    it("copies every file, including a component's own directory", function()
      core.on_command({ "copy", "Alpha", "confirm" })
      assert.is_not_nil(env.fs.files["data/Azureblood/bar.lua"]:find("x = 40", 1, true))
      assert.are.equal('return { snap = 25, slot = "default" }', env.fs.files["data/Azureblood/core.lua"])
      assert.are.equal("return { deep = true }", env.fs.files["data/Azureblood/bar/extra.lua"])
    end)

    it("reloads and re-applies the configuration it just copied", function()
      core.on_command({ "copy", "Alpha", "confirm" })
      assert.are.same({ 40, 50 }, widget.pos)
      assert.is_not_nil(widget.config, "the component is re-attached to the new config")
    end)

    it("matches the source character case-insensitively", function()
      core.on_command({ "copy", "alpha", "confirm" })
      assert.are.same({ 40, 50 }, widget.pos)
    end)

    it("lists the characters it does know when the source is unknown", function()
      core.on_command({ "copy", "Nobody", "confirm" })
      local said = env.said()
    ...
    it("refuses to copy a character onto itself, whatever the case", function()
      core.on_command({ "copy", "azureblood", "confirm" })
      assert.is_not_nil(env.said():lower():find("already"))
    end)

    it("copies the file for every component, not just the first", function()
      env.fs.put("data/Alpha/clock.lua", "return { slots = { default = { pos = { x = 70, y = 80 } } } }")
      local clock = core.register(fakes.widget("clock", slot_defaults(10, 20)))
      core.on_command({ "copy", "Alpha", "confirm" })
      assert.are.same({ 40, 50 }, widget.pos)
      assert.are.same({ 70, 80 }, clock.pos)
    end)

    it("says so when a file could not be copied, instead of claiming success", function()
      env.fs.fail_write_paths["data/Azureblood/bar.lua"] = true
      core.on_command({ "copy", "Alpha", "confirm" })
      local said = env.said():lower()
      assert.is_not_nil(said:find("could not"), "said: " .. env.said())
      assert.is_nil(said:find("configuration reloaded"), "must not claim a clean copy")
    end)

    it("counts an unreadable source file as a failure", function()
      env.fs.files["data/Alpha/bar.lua"] = false
      core.on_command({ "copy", "Alpha", "confirm" })
    ...
    it("does not offer the current character as a source", function()
      core.on_command({ "copy", "Nobody", "confirm" })
      local listed = env.said():match("known: ([^\n]+)") or ""
      assert.is_not_nil(listed:find("Alpha", 1, true))
      assert.is_nil(listed:find("Azureblood", 1, true), "listed: " .. listed)
    end)

    it("says it merges, because a file the source lacks is left alone", function()
      core.on_command({ "hide", "bar" })
      env.fs.files["data/Alpha/bar.lua"] = nil
      core.on_command({ "copy", "Alpha", "confirm" })
      assert.is_false(widget.shown, "bar.lua was not in the source, so it survives")
    end)

    it("needs a character before it will copy", function()
      core.on_logout()
      core.on_command({ "copy", "Alpha", "confirm" })
      assert.is_not_nil(env.said():lower():find("log in"))
    end)
