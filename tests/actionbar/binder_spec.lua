...
local function build(opts)
  opts = opts or {}
  -- `hotbar = true` builds the same binder as the hotbar does: its render,
  -- its grammar, its one-sided store.
  local hotbar = opts.hotbar == true
  local config = hotbar and require("components/hotbar/defaults")(1920, 1080) or build_defaults(1920, 1080)
  for _, set in ipairs(opts.shared_sets or {}) do
    config.set_flags[set].shared = true
  end
  local prims = fakes.prims()
  local render = (hotbar and require("components/hotbar/render") or new_render)({
    config = config,
  ...
  local bindings = new_bindings({
    geometry = hotbar and require("lib/actionbar/grammar").hotbar() or nil,
    load = function(name)
      return files[name]
    end,
  ...
  local binder = new_binder({
    name = hotbar and "hotbar" or nil,
    grammar = hotbar and require("lib/actionbar/grammar").hotbar() or nil,
...
describe("crossbar binder", function()
  ...
  describe("as the hotbar's binder", function()
    it("names the slot in the hotbar's grammar and speaks as the hotbar", function()
      local binder, env = build({
        hotbar = true,
        files = { WAR = { sets = { [1] = { row = { [3] = { type = "ja", action = "Provoke" } } } } } },
        groups = function()
          return { { key = "bar1", x = 100, y = 500, scale = 1, set = 1, side = "row", rows = 1 } }
        end,
      })
      binder.open()
      local x, y = env.render.slot_pos(1, 7)
      click(binder, 100 + x + 5, 500 + y + 5)
      assert.is_not_nil(shown_text(env, "1:7"), "the subhead names the slot as the CLI would")
      assert.is_nil(shown_text(env, "1R7"))
      click(binder, centre(row_named(env, "base")))
      pick(binder, env, "Berserk")
      assert.is_not_nil(
        env.said[1]:find("^hotbar: bound Berserk %-> base / set 1 / row / slot 7"),
  ...
  describe("the submenu step", function()
    ...
    --[[ The other way out of a nested view without a rebuild: a SUBJOB change
         (or an unequip) takes the layer under the cursor out of reach, which
         strands the wizard back at the layer step. `refresh` compares the main
         job alone, so nothing rebuilds the catalog on that path. ]]
    it("drops the submenu when the layer under the cursor goes out of reach", function()
      local binder, env = build({ catalog = stratagems() })
      open_stack(binder, env, "left", 3)
      click(binder, centre(row_named(env, "ctx:light-arts")))
      click(binder, centre(entry_named(env, "Stratagems")))
      assert.are.equal("Stratagems", binder.catalog_view().submenu)
      -- /NIN cannot reach Light Arts, so the cursor's layer is gone.
      env.bindings.set_job("WAR", "NIN")
      binder.refresh()
