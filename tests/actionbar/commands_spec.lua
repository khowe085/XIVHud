...
describe("crossbar commands", function()
  ...
  describe("as the hotbar's CLI", function()
    ...
    it("binds, lists and swaps by <set>:<slot>, in its own name", function()
      local commands, world = hotbar({ known = { Cure = true } })
      local reply = commands.command({ "bind", "3:7", "ma", "Cure", "t" })
      assert.is_not_nil(reply:find("^hotbar: bound 3:7"), reply)
      assert.are.same({ type = "ma", action = "Cure", target = "t" }, stored(world, "SCH", 3, "row", 7))
      local lines = commands.command({ "list", "3" })
      assert.are.equal("hotbar: SCH bindings", lines[1])
      assert.is_not_nil(lines[3]:find("3:7", 1, true), lines[3])
      commands.command({ "swap", "3:7", "3:10" })
      assert.are.same({ type = "ma", action = "Cure", target = "t" }, stored(world, "SCH", 3, "row", 10))
      assert.is_nil(stored(world, "SCH", 3, "row", 7))
    end)

    it("refuses the crossbar's shape and an eleventh slot, naming its own form", function()
      local commands = hotbar({ known = { Cure = true } })
      local reply = commands.command({ "bind", "3L7", "ma", "Cure", "t" })
      assert.is_not_nil(reply:find("<set>:<slot>", 1, true), reply)
      reply = commands.command({ "bind", "3:11", "ma", "Cure", "t" })
      assert.is_not_nil(reply:find("1%-10"), reply)
    end)

    it("lists the lines its widget asks for in help", function()
      local commands =
    ...
    it("has no views and no wxhb", function()
      local commands = hotbar()
      assert.is_false(commands.handles("view"))
      assert.is_false(commands.handles("wxhb"))
      local help = commands.command({ "help" })
      assert.are.equal("hotbar commands:", help[1])
      for _, line in ipairs(help) do
        assert.is_nil(line:find("view", 1, true), line)
        assert.is_nil(line:find("wxhb", 1, true), line)
        assert.is_nil(line:find("crossbar", 1, true), line)
      end
    end)
  end)
end)
