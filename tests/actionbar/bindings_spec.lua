...
describe("crossbar bindings", function()
  ...
  describe("the job gate on contexts", function()
    ...
    it("treats a context naming no job as reachable everywhere", function()
      local roster_key = "lib/actionbar/contexts"
      local bindings_key = "lib/actionbar/bindings"
  ...
  describe("cycle", function()
    ...
    it("walks the rotation backwards on cycle(-1), wrapping, skipping what the forward walk skips", function()
      local bindings = kevins()
      bindings.set_job("WAR")
      bindings.set_weapon_state("drawn")
      assert.equal(1, bindings.active_set())
      assert.equal(2, bindings.cycle(-1), "back from 1 wraps to the last drawn set")
      assert.equal(1, bindings.cycle(-1))
      bindings.set_weapon_state("sheathed")
      assert.equal(6, bindings.active_set())
      assert.equal(8, bindings.cycle(-1))
      assert.equal(7, bindings.cycle(-1))
      assert.equal(8, bindings.cycle(0), "a zero step is a forward walk")
    end)
  ...
  describe("over the hotbar's geometry", function()
    local grammar = require("lib/actionbar/grammar")

    it("keys one side of ten slots and refuses the crossbar's sides", function()
      local bindings, world = build({ geometry = grammar.hotbar() })
      bindings.set_job("SCH")
      assert.is_true(bindings.bind("3", "row", 10, record("Cure")))
      assert.are.same(record("Cure"), world.files.SCH.sets[3].row[10])
      assert.are.same(record("Cure"), (bindings.resolve(3, "row", 10)))
      assert.is_nil(bindings.resolve(3, "left", 1))
      local ok, err = bindings.bind("3", "row", 11, record("Cure"))
      assert.is_nil(ok)
      assert.is_not_nil(err:find("10", 1, true), err)
      ok, err = bindings.bind("3", "left", 1, record("Cure"))
      assert.is_nil(ok)
      assert.is_not_nil(err, "a side this geometry does not have")
    end)

    it("lands and cycles over the row", function()
      local bindings = build({ geometry = grammar.hotbar() })
      bindings.set_job("SCH")
      bindings.bind("2", "row", 4, record("Cure"))
