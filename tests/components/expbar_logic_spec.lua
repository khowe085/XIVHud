...
describe("expbar logic", function()
  ...
  describe("geometry", function()
    it("hangs the icon off the left of a flush header and bar", function()
      ...
      assert.are.same({ x = 120, y = 200 }, { x = geometry.header.x, y = geometry.header.y })
      assert.are.equal(120, geometry.background.x)
      assert.are.equal(218, geometry.background.y)
      assert.are.equal(390, geometry.background.width)
      assert.are.equal(5, geometry.background.height)
    end)

    it("drops the bar below an icon taller than the header band", function()
      config.job_icon.size = 30
      assert.are.equal(232, logic.geometry(100, 200, 1).background.y)
      local _, _, _, height = logic.bounds(100, 200, 1)
      assert.are.equal(37, height)
    end)

    it("insets the fill inside the frame", function()
      local geometry = logic.geometry(100, 200, 1)
      assert.are.equal(122, geometry.fill.x)
      assert.are.equal(218, geometry.fill.y)
    end)
    ...
    it("keeps the header still when there is no icon to draw", function()
      ...
      -- line left and back again as the player is filled in field by field.
      assert.is_nil(logic.job_icon())
      assert.are.equal(120, logic.geometry(100, 200, 1).header.x)
    end)
    ...
    it("multiplies every measure by the scale", function()
      ...
      assert.are.equal(140, geometry.header.x)
      assert.are.equal(236, geometry.background.y)
      assert.are.equal(780, geometry.background.width)
      assert.are.equal(10, geometry.background.height)
      assert.are.equal(144, geometry.fill.x)
      assert.are.equal(22, geometry.font_size)
    end)

    it("rounds the font size to a whole pixel, which is all a prim can draw", function()
      assert.are.equal(17, logic.geometry(100, 200, 1.5).font_size)
    end)
    ...
    it("reserves the whole box, from the origin it was given", function()
      ...
      assert.are.same({ 820, 46 }, { wide, tall })
    end)
  end)
  ...
  describe("the bar", function()
    ...
    it("eases down as well as up, which a level up and a preview exit both do", function()
      ...
      local plan = logic.tick(0)
      assert.is_true(plan.width < 386, "the bar must come down")
    ...
    it("never draws past the end of the frame", function()
      ...
      assert.are.equal(386, settle().width)
    end)

    it("converges and then stops redrawing", function()
      logic.on_packet(CHAR_STATS, char_stats({ ["Current EXP"] = 2500, ["Required EXP"] = 5000 }))
      local plan = settle()
      assert.are.equal(193, plan.width)
