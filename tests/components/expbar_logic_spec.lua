...
describe("expbar logic", function()
  ...
  describe("geometry", function()
    it("stands the icon left of a flush header and bar", function()
      ...
      assert.are.equal(119, geometry.header.x)
      assert.are.equal(200, geometry.header.y)
      assert.are.equal(119, geometry.background.x)
      assert.are.equal(213, geometry.background.y)
      assert.are.equal(286, geometry.background.width)
    ...
    it("spans both rows with the icon", function()
      ...
      local geometry = logic.geometry(100, 200, 1)
      assert.are.equal(18, geometry.icon.size)
      assert.are.equal(218, geometry.icon.y + geometry.icon.size)
      assert.are.equal(218, geometry.background.y + geometry.background.height)
    end)

    it("spans them at scale too", function()
      local geometry = logic.geometry(100, 200, 2)
      assert.are.equal(36, geometry.icon.size)
      assert.are.equal(236, geometry.icon.y + geometry.icon.size)
      assert.are.equal(236, geometry.background.y + geometry.background.height)
    end)
    ...
    it("grows the box for an icon taller than the rows it spans", function()
      ...
      assert.are.equal(213, logic.geometry(100, 200, 1).background.y)
    ...
    it("insets the fill inside the frame", function()
      local geometry = logic.geometry(100, 200, 1)
      assert.are.equal(121, geometry.fill.x)
      assert.are.equal(213, geometry.fill.y)
    end)
    ...
    it("keeps the header still when there is no icon to draw", function()
      ...
      assert.is_nil(logic.job_icon())
      assert.are.equal(119, logic.geometry(100, 200, 1).header.x)
      assert.are.equal(119, logic.geometry(100, 200, 1).background.x)
    end)

    it("multiplies every measure by the scale", function()
      local geometry = logic.geometry(100, 200, 2)
      assert.are.equal(138, geometry.header.x)
      assert.are.equal(226, geometry.background.y)
      assert.are.equal(572, geometry.background.width)
      assert.are.equal(10, geometry.background.height)
      assert.are.equal(142, geometry.fill.x)
      assert.are.equal(16, geometry.font_size)
    end)

    it("rounds the font size to a whole pixel, which is all a prim can draw", function()
      assert.are.equal(12, logic.geometry(100, 200, 1.5).font_size)
    end)
    ...
    it("reserves the whole box, from the origin it was given", function()
      ...
      assert.are.same({ 610, 36 }, { wide, tall })
    end)
  end)
  ...
  describe("the bar", function()
    ...
    it("eases down as well as up, which a level up and a preview exit both do", function()
      ...
      local plan = logic.tick(0)
      assert.is_true(plan.width < 282, "the bar must come down")
    ...
    it("never draws past the end of the frame", function()
      ...
      assert.are.equal(282, settle().width)
    end)

    it("converges and then stops redrawing", function()
      logic.on_packet(CHAR_STATS, char_stats({ ["Current EXP"] = 2500, ["Required EXP"] = 5000 }))
      local plan = settle()
      assert.are.equal(141, plan.width)
