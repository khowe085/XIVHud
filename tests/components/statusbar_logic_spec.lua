...
describe("statusbar logic", function()
  ...
  local function build()
    ...
    config = { bars = defaults.bars, priority = defaults.priority, timers = defaults.timers, tooltips = true }
  ...
  describe("tooltips", function()
    ...
    it("names the buff with its id", function()
      assert.are.equal("haste (33)", logic.tooltip(HASTE))
      assert.are.equal("buff 9998 (9998)", logic.tooltip(9998))
    end)

    it("names nothing while tooltips are switched off", function()
      config.tooltips = false
      assert.is_nil(logic.tooltip(HASTE))
    end)
  end)
  ...
  describe("commands", function()
    ...
    -- The buff and filter verbs arrive through `//hud buffs statusbar`.
    local function buffs(...)
      local lines, changed = logic.buff_command({ ... })
      return table.concat(lines, "\n"), changed
    end
    ...
    it("names the four filters when given one it does not know", function()
      local said, changed = buffs("filter", "wobble")
    ...
    it("edits a bar's own filter list", function()
      local _, changed = buffs("bar2", "filter", "add", "doom")
      assert.is_true(changed)
      assert.are.same({ DOOM }, config.bars.bar2.filters)
      buffs("bar2", "filter", "mode", "whitelist")
      assert.are.equal("whitelist", config.bars.bar2.filter_mode)
      assert.is_not_nil(buffs("bar2", "filter", "list"):find("doom", 1, true))
      buffs("bar2", "filter", "clear")
      assert.are.same({}, config.bars.bar2.filters)
    end)

    it("lists a bar's filters when filter is given nothing", function()
      assert.is_not_nil(buffs("filter"):find("nothing", 1, true))
    end)

    it("names the bar in a filter message", function()
      assert.is_not_nil(buffs("bar2", "filter", "mode", "greylist"):find("statusbar bar2 filter", 1, true))
    end)
    ...
    it("switches the tooltips", function()
      local _, changed = say("tooltips", "off")
      assert.is_true(changed)
      assert.is_false(config.tooltips)
      say("tooltips", "on")
      assert.is_true(config.tooltips)
      assert.is_false(select(2, say("tooltips", "maybe")))
      assert.is_not_nil(say():find("tooltips on", 1, true))
    end)

    it("refuses a bar word in front of tooltips too", function()
      local said, changed = say("bar2", "tooltips", "off")
      assert.is_false(changed)
    ...
    it("edits the shared priority through the buff verbs", function()
      local _, changed = buffs("top", "doom")
      assert.is_true(changed)
      assert.are.equal(1, config.priority[DOOM])
      assert.is_not_nil(buffs("list"):find("page 1/", 1, true))
      assert.is_not_nil(buffs("find", "weak"):find("weakness", 1, true))
    end)

    -- The old paths are gone outright: `buff` and `filter` are no longer
    -- verbs of the plain command, and fall to its hint like any other word.
    it("no longer answers buff or filter as plain command verbs", function()
      local said, changed = say("buff", "top", "doom")
      assert.is_false(changed)
      assert.is_nil(config.priority[DOOM])
      assert.is_not_nil(said:find("buff", 1, true))
      said, changed = say("filter", "debuffs")
      assert.is_false(changed)
      assert.are.equal("all", config.bars.bar1.filter)
      assert.is_nil(said:find("filter,", 1, true), "hint: " .. said)
    end)
    ...
    it("refuses a bar word in front of an order verb, which is one order for all", function()
      local said, changed = buffs("bar2", "top", "doom")
      assert.is_false(changed)
      assert.is_not_nil(said:find("bar", 1, true))
      assert.is_nil(config.priority[DOOM])
    end)

    -- Only a real order verb earns the "one order for every bar" refusal;
    -- anything else after a bar word is unknown and gets the verb hint.
    it("answers an unknown verb after a bar word with the hint, not the shared-order refusal", function()
      local said, changed = buffs("bar2", "wobble")
      assert.is_false(changed)
      assert.is_nil(said:find("one order", 1, true), "said: " .. said)
      assert.is_not_nil(said:find("//hud buffs statusbar", 1, true))
      assert.is_not_nil(said:find("top", 1, true))
    end)

    it("edits the first bar's filter list when no bar is named", function()
      local _, changed = buffs("filter", "add", "doom")
      assert.is_true(changed)
      assert.are.same({ DOOM }, config.bars.bar1.filters)
    end)

    it("answers an unknown buff verb with a hint naming filter among the verbs", function()
      local said, changed = buffs("wobble")
      assert.is_false(changed)
      assert.is_not_nil(said:find("//hud buffs statusbar", 1, true))
    ...
    it("refuses words after the bar in a buff listing rather than dropping them", function()
      local said, changed = buffs("bar2", "list")
      assert.is_false(changed)
    ...
    it("names a broken bar in the listing rather than skipping it", function()
      ...
      assert.is_not_nil(buffs("bar2"):find("reset", 1, true))
      local all = buffs()
    ...
    it("says so when a bar has nothing to draw", function()
      logic.set_buffs({})
      assert.is_not_nil(buffs():find("nothing", 1, true))
    end)
    ...
    it("answers an unknown verb with a hint naming the verbs", function()
      ...
      assert.is_nil(said:find("filter", 1, true), "hint: " .. said)
      assert.is_not_nil(said:find("tooltips", 1, true))
    end)
