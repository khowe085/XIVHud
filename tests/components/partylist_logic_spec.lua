...
describe("partylist logic", function()
  ...
  describe("buff icons", function()
    ...
    it("never promises more icons than the layout has slots for", function()
      ...
      assert.are.equal(16, #shown(many))
      assert.is_not_nil(table.concat(select(1, logic.buff_command({})), "\n"):find("first 16", 1, true))
    end)
  ...
  describe("commands", function()
    ...
    -- The buff verbs arrive through `//hud buffs partylist`, the list word
    -- already stripped by the widget.
    ...
    describe("buff priority", function()
      ...
      it("clamps a page number that is off either end", function()
        local pages = buffs("list"):match("page %d+/(%d+)")
        assert.is_not_nil(pages)
        assert.is_not_nil(buffs("list", "9999"):find("page " .. pages .. "/" .. pages, 1, true))
        assert.is_not_nil(buffs("list", "0"):find("page 1/" .. pages, 1, true))
      end)

      it("marks where the icon cap cuts the order", function()
        assert.is_not_nil(buffs("list", "1"):find("cut", 1, true))
      end)

      it("finds a buff by part of its name", function()
        local said = buffs("find", "weak")
      ...
      it("finds a buff whose name has spaces", function()
        assert.is_not_nil(buffs("find", "max", "hp"):find("max hp boost", 1, true))
      end)

      it("says so when a search matches nothing", function()
        local said, changed = buffs("find", "zzzz")
        assert.is_false(changed)
      ...
      it("reports the rank a buff actually landed on, not the one asked for", function()
        local said = buffs("rank", "doom", "9999")
        assert.is_nil(said:find("9999", 1, true))
      end)

      it("rejects a rank that is not a positive whole number", function()
        assert.is_false(select(2, buffs("rank", "doom", "0")))
        assert.is_false(select(2, buffs("rank", "doom", "half")))
        assert.is_false(select(2, buffs("rank", "doom")))
      end)
      ...
      it("refuses an ambiguous name and lists the candidates", function()
        ...
        local said, changed = buffs("top", "sleep")
      ...
      describe("the live buffs on a member", function()
        ...
        it("defaults to the player themselves", function()
          assert.is_not_nil(buffs("active"):find("weakness", 1, true))
        end)

        it("names a member's buffs, cap or no cap", function()
          config.buffs.max_icons = 1
          local said = buffs("active", "volker")
          assert.is_not_nil(said:find("KO", 1, true))
        ...
        it("says so for a member no buff packet has covered", function()
          logic.apply_buffs(2, {})
          assert.is_not_nil(buffs("active", "volker"):find("no buffs", 1, true))
        end)

        it("says so for a name that is not in the party", function()
          assert.is_not_nil(buffs("active", "nobody"):find("not in", 1, true))
        end)
      end)
      ...
      it("complains rather than erroring when the buff settings are not a table", function()
        ...
        local said, changed = buffs("top", "doom")
        assert.is_false(changed)
        assert.is_not_nil(said:find("reset", 1, true))
        assert.is_false(select(2, buffs("filter", "list")))
      end)

      it("is not a verb of the plain command any more", function()
        local said, changed = say("buff", "top", "doom")
        assert.is_false(changed)
        assert.is_nil(said:find("buff,", 1, true), "hint: " .. said)
        assert.is_nil(said:find("or buff", 1, true), "hint: " .. said)
      end)
