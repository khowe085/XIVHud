...
- **Character scoping:** the character name is unknown until login, so components are attached on login and detached on logout, and nothing renders while logged out.- **The name is not enough — wait for the vitals.** parambar was seen in a live client reading 0 HP / 0 MP from login until the first action: the `hp change`-style events only fire on an actual *change*, so a component attached before the client filled the player in keeps those zeros. Core therefore also waits for `get_player().vitals.max_hp` to be nonzero (nonzero for any loaded character, a dead one included). That the name lands first is a diagnosis and `max_hp` is wiki-sourced, unconfirmed live, so the wait is bounded: after three seconds core attaches on the name alone and says so. Only logging out resets the clock — a timer any flicker rewinds is not a bound. The wait is skipped entirely when the registry is empty, which is safe mode: nothing there can latch a zero, and it has no render loop to retry from.
